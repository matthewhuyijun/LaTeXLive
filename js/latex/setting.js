export var setting = {
  ...
  init: function (opt) {
    ...
    // 移除不再支持的主题
    if (setting.map_theme.blue) {
      delete setting.map_theme.blue;
    }
    if (setting.map_theme.pink) {
      delete setting.map_theme.pink;
    }
    if (setting.map_theme.simple) {
      delete setting.map_theme.simple;
    }
    setting.fn_refresh = opt.fn_refresh;
    setting.getLocalStorageSetting();
    // Ensure required MathJax extensions are enabled by default (merge for existing users)
    (function ensureDefaultTexExtensionsEnabled() {
      try {
        var required = [
          "physics",
          "mhchem",
        ...
        var existing = Array.isArray(setting.localData.extend) ? setting.localData.extend.slice() : [];
        for (var i = 0; i < required.length; i++) {
          if (existing.indexOf(required[i]) === -1) {
            existing.push(required[i]);
          }
        }
        setting.localData.extend = existing;
        setting.setLocalStorageSetting();
      } catch (e) {}
    })();
    // 如果本地存储的主题已被移除，则回退为 default 并写回存储
    if (!setting.map_theme.hasOwnProperty(setting.localData.theme)) {
      setting.localData.theme = "default";
      setting.setLocalStorageSetting();
    }
    setting.init_theme();
  ...
  init_theme: function () {
    ...
    let currentRadio = document.getElementById("radio-" + currenttheme);
    if (currentRadio) {
      ...
      setting.localData.theme = "default";
      setting.setLocalStorageSetting();
      let defaultRadio = document.getElementById("radio-default");
      if (defaultRadio) defaultRadio.checked = true;
      setting.obj_theme.currentTheme = "default";
    }
  },
